...
data = read_csv('./ticket-classifier/data/all_tickets.csv')
...
ticket_tokens <- data_tt_1 %>%
    unnest_tokens(output = words, input = body)

# Output tidyDF of tokens for future stopword/word stemmer removal
write.csv(ticket_tokens, file='./ticket-classifier/data/tokens.csv')

# Output raw data minus the unused coulumns and filtered
write.csv(data_tt_1, file='./ticket-classifier/data/filtered_data_1.csv')
